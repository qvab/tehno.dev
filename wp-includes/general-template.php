		...
		if ( 'html5' == $format ) {
			$form = '<form role="search" ' . $aria_label . 'method="get" class="search-form" action="' . esc_url( home_url( '/' ) ) . '">
				<label>
					...
					<input type="search" class="search-field" placeholder="' . esc_attr_x( IS_SITE_LANG_EN ? 'Search &hellip;' : 'Поиск &hellip;', 'placeholder' ) . '" value="' . get_search_query() . '" name="s" />
				</label>
				<input type="submit" class="search-submit" value="' . esc_attr_x( IS_SITE_LANG_EN ? 'Search' : 'Поиск', 'submit button' ) . '" />
			</form>';
		} else {
			$form = '<form role="search" ' . $aria_label . 'method="get" id="searchform" class="searchform" action="' . esc_url( home_url( '/' ) ) . '">
				<div>
					...
					<input type="submit" id="searchsubmit" value="' . esc_attr_x( IS_SITE_LANG_EN ? 'Search' : 'Поиск', 'submit button' ) . '" />
				</div>
			</form>';
		}
	}
...
function get_bloginfo( $show = '', $filter = 'raw' ) {
    $isLandEn = $_COOKIE["wp-wpml_current_language"] == "ru" ? 0 : 1;
	switch ( $show ) {
		...
		case 'name':
		default:
			$output = $isLandEn ? "Tehno.guru - our site about Tools & Home Improvement" /*get_option('blogname_en')*/ : get_option('blogname');
			break;
	}
